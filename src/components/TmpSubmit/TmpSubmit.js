    ...
    <section>
      ...
      <div>
        <div style={{ border: '1px solid #000' }}>
          {!preview && <textarea ref={textareaRef} value={content} onChange={handleContentChange} style={{ width: '100%', display: 'block' }} />}
          {preview && <div dangerouslySetInnerHTML={{ __html: markup(preview) }}></div>}
        </div>
        {!preview && <button onClick={handlePreview}>preview</button>}
        {preview && <button onClick={handleEdit}>Edit</button>}
        <button>save</button>
